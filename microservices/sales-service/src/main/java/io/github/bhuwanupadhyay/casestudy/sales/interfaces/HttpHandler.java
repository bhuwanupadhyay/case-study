...
public interface HttpHandler {
	...
	static List<OrderItem> toOrderItems(List<OrderItemResource> orderItems) {
		return orderItems.stream().map(e -> new OrderItem(e.itemId(), e.quantity())).collect(Collectors.toList());
	}
	...
	ResponseEntity<Void> modifyOrder(@PathVariable("orderId") String orderId, @RequestBody ModifyOrderRequest request);

	@PostMapping("/sales/{orderId}/cancel")
	ResponseEntity<Void> cancelOrder(@PathVariable("orderId") String orderId, @RequestBody CancelOrderRequest request);
	...
	record OrderItemResource(@JsonProperty("itemId")String itemId, @JsonProperty("quantity")Integer quantity) {
	}
