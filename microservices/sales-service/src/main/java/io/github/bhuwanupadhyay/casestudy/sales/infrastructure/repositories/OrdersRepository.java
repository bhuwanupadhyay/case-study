...
import io.github.bhuwanupadhyay.casestudy.sales.domain.model.valueobjects.*;
...
public class OrdersRepository extends Orders {
	...
	public Optional<Order> findOne(OrderId orderId) {
		SaleOrdersRecord record = context.selectFrom(SALE_ORDERS).where(ORDERS.ORDER_ID.eq(orderId.value())).fetchOne();
	...
	protected void persist(Order entity) {
		...
		if (count == 0) {
			...
			context.insertInto(SALE_ORDERS).set(ORDERS.ORDER_ID, entity.getId().value())
					.set(ORDERS.CUSTOMER_ID, entity.getCustomerId().value())
					.set(ORDERS.STATUS, entity.getStatus().name()).set(ORDERS.CREATED_AT, entity.getCreatedAt())
					.execute();
			for (Map.Entry<ItemId, Quantity> e : entity.getOrderLines().entrySet()) {
				context.insertInto(ORDER_LINES).set(ORDER_LINES.ITEM_ID, e.getKey().value())
						.set(ORDER_LINES.QUANTITY, e.getValue().value())
						.set(ORDER_LINES.ORDER_ID, entity.getId().value()).execute();
			}
		}
		else {
			context.update(ORDERS).set(ORDERS.STATUS, entity.getStatus().name())
					.set(ORDERS.CANCELLATION_REASON, entity.getCancellationReason().value())
					.where(ORDERS.ORDER_ID.eq(entity.getId().value())).execute();
			for (Map.Entry<ItemId, Quantity> e : entity.getOrderLines().entrySet()) {
				int itemCount = context.fetchCount(ORDER_LINES, ORDER_LINES.ORDER_ID.eq(entity.getId().value()),
				...
				if (itemCount == 0) {
					...
					context.update(ORDER_LINES).set(ORDER_LINES.QUANTITY, e.getValue().value())
	...
	private Order toOrder(SaleOrdersRecord record) {
		...
		Order order = new Order(new OrderId(record.get(ORDERS.ORDER_ID)))
				...
				.withCancellationReason(new CancellationReason(record.get(ORDERS.CANCELLATION_REASON)));

		order.setCreatedAt(record.get(ORDERS.CREATED_AT));

		Result<SaleOrderLinesRecord> result = context.selectFrom(ORDER_LINES)
				.where(ORDER_LINES.ORDER_ID.eq(order.getId().value())).fetch();
