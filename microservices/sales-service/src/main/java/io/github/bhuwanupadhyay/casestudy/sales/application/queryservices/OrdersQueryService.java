...
public record OrdersQueryService(DSLContext context) {
	...
	public OrderResource findByOrderId(String orderId) {
		Result<Record3<String, String, Integer>> orderById = this.context
				.select(ORDERS.ORDER_ID, ORDER_LINES.ITEM_ID, ORDER_LINES.QUANTITY).from(ORDER_LINES).join(ORDERS)
				.on(ORDERS.ORDER_ID.eq(ORDER_LINES.ORDER_ID)).where(ORDERS.ORDER_ID.eq(orderId)).fetch();
	...
	public List<OrderResource> findAll() {
		Map<String, List<Record3<String, String, Integer>>> groupByOrderId = this.context
				.select(ORDERS.ORDER_ID, ORDER_LINES.ITEM_ID, ORDER_LINES.QUANTITY).from(ORDER_LINES).join(ORDERS)
				.on(ORDERS.ORDER_ID.eq(ORDER_LINES.ORDER_ID)).fetch().stream()
